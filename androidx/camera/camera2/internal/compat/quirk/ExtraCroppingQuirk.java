...
public class ExtraCroppingQuirk implements Quirk {
    ...
    public Size getVerifiedResolution(SurfaceConfig.ConfigType configType) {
        ...
        if (i == 2) {
            return new Size(SocketMessages.PayloadType.SC_LIVE_BULLET_CLOUD_PLAY_OPERATION_TYPE_NOTICE, 720);
        }
        if (i != 3) {
            return null;
        }
    ...
    private static boolean isSamsungDistortion() {
        if (!(DeviceUtils.ROM_SAMSUNG.equalsIgnoreCase(Build.BRAND) && SAMSUNG_DISTORTION_MODELS_TO_API_LEVEL_MAP.containsKey(Build.MODEL.toUpperCase(Locale.US)))) {
            return false;
        }
