                ...
                if (!CombinedChannelDuplexHandler.this.outboundCtx.removed) {
                    try {
                        ...
                        if (CombinedChannelDuplexHandler.logger.isWarnEnabled()) {
                            CombinedChannelDuplexHandler.logger.warn("An exception was thrown by a user handler's exceptionCaught() method while handling the following exception:", th2);
                        }
                    }
                } else {
    ...
    static class DelegatingChannelHandlerContext implements ChannelHandlerContext {
        ...
        final void remove() {
            ...
            if (eventExecutorExecutor.inEventLoop()) {
                ...
                eventExecutorExecutor.execute(new OneTimeTask() { // from class: io.netty.channel.CombinedChannelDuplexHandler.DelegatingChannelHandlerContext.1
