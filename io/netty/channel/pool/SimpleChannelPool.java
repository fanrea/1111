...
public class SimpleChannelPool implements ChannelPool {
    ...
    public SimpleChannelPool(Bootstrap bootstrap, final ChannelPoolHandler channelPoolHandler, ChannelHealthChecker channelHealthChecker, boolean z) {
        ...
        this.bootstrap = ((Bootstrap) ObjectUtil.checkNotNull(bootstrap, "bootstrap")).mo1516clone();
    ...
    private Future<Channel> acquireHealthyFromPoolOrNew(final Promise<Channel> promise) {
        final Channel channelPollChannel;
        try {
            channelPollChannel = pollChannel();
        } catch (Throwable th) {
            promise.setFailure(th);
        }
        if (channelPollChannel == null) {
            Bootstrap bootstrapMo1516clone = this.bootstrap.mo1516clone();
            bootstrapMo1516clone.attr(POOL_KEY, this);
            ChannelFuture channelFutureConnectChannel = connectChannel(bootstrapMo1516clone);
