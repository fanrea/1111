...
public class LocalChannel extends AbstractChannel {
    ...
    static {
        AtomicReferenceFieldUpdater<LocalChannel, Future> atomicReferenceFieldUpdaterNewAtomicReferenceFieldUpdater = PlatformDependent.newAtomicReferenceFieldUpdater(LocalChannel.class, "finishReadFuture");
        if (atomicReferenceFieldUpdaterNewAtomicReferenceFieldUpdater == null) {
            atomicReferenceFieldUpdaterNewAtomicReferenceFieldUpdater = AtomicReferenceFieldUpdater.newUpdater(LocalChannel.class, Future.class, "finishReadFuture");
        }
        FINISH_READ_FUTURE_UPDATER = atomicReferenceFieldUpdaterNewAtomicReferenceFieldUpdater;
    ...
    public LocalChannel() {
        ...
        this.inboundBuffer = PlatformDependent.newMpscQueue();
        this.readTask = new Runnable() { // from class: io.netty.channel.local.LocalChannel.1
            @Override // java.lang.Runnable
            public void run() {
                ChannelPipeline channelPipelinePipeline = LocalChannel.this.pipeline();
                while (true) {
                    Object objPoll = LocalChannel.this.inboundBuffer.poll();
                    if (objPoll != null) {
    ...
    @Override // io.netty.channel.AbstractChannel
    public void doWrite(ChannelOutboundBuffer channelOutboundBuffer) throws ClosedChannelException {
                ...
                if (objCurrent != null) {
                    try {
                        if (localChannel.state == State.CONNECTED) {
                            ...
                            channelOutboundBuffer.remove(CLOSED_CHANNEL_EXCEPTION);
                        }
                    } catch (Throwable th) {
    ...
    private void runFinishPeerReadTask(final LocalChannel localChannel) {
        ...
        try {
            if (localChannel.writeInProgress) {
                localChannel.finishReadFuture = localChannel.eventLoop().submit((Runnable) oneTimeTask);
            } else {
    ...
    public void finishPeerRead0(LocalChannel localChannel) {
        ...
        ChannelPipeline channelPipelinePipeline = localChannel.pipeline();
        if (!localChannel.readInProgress) {
            return;
        }
        localChannel.readInProgress = false;
        while (true) {
            Object objPoll = localChannel.inboundBuffer.poll();
            if (objPoll != null) {
                channelPipelinePipeline.fireChannelRead(objPoll);
            } else {
                channelPipelinePipeline.fireChannelReadComplete();
