...
class PooledHeapByteBuf extends PooledByteBuf<byte[]> {
    private static final Recycler<PooledHeapByteBuf> RECYCLER = new Recycler<PooledHeapByteBuf>() { // from class: io.netty.buffer.PooledHeapByteBuf.1
        ...
        public final PooledHeapByteBuf newObject2(Recycler.Handle<PooledHeapByteBuf> handle) {
    ...
    PooledHeapByteBuf(Recycler.Handle<? extends PooledHeapByteBuf> handle, int i) {
        super(handle, i);
    }
    ...
    private int getBytes(int i, GatheringByteChannel gatheringByteChannel, int i2, boolean z) {
        ...
        if (z) {
            ...
            byteBufferWrap = ByteBuffer.wrap((byte[]) this.memory);
        }
        return gatheringByteChannel.write((ByteBuffer) byteBufferWrap.clear().position(iIdx).limit(iIdx + i2));
    }
    ...
    public final ByteBuf setBytes(int i, ByteBuffer byteBuffer) {
        ...
        byteBuffer.get((byte[]) this.memory, idx(i), iRemaining);
        return this;
    }
    ...
    public final int setBytes(int i, ScatteringByteChannel scatteringByteChannel, int i2) {
        ...
        int iIdx = idx(i);
        try {
            return scatteringByteChannel.read((ByteBuffer) internalNioBuffer().clear().position(iIdx).limit(iIdx + i2));
    ...
    public final ByteBuf copy(int i, int i2) {
        checkIndex(i, i2);
        ByteBuf byteBufHeapBuffer = alloc().heapBuffer(i2, maxCapacity());
        byteBufHeapBuffer.writeBytes((byte[]) this.memory, idx(i), i2);
