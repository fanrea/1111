...
public class PluginResources extends Resources {
    ...
    public static AssetManager appendHostRes(Resources resources) {
        String strD = h.d(Zeus.getAppApplication());
        String strHc = h.hc(Zeus.getAppApplication());
        List<String> listHc = mk.hc();
        d dVar = new d();
        AssetManager assets = resources.getAssets();
        AssetManager assets2 = Zeus.getAppApplication().getAssets();
        HashSet hashSet = new HashSet(mk.d(assets));
        List<String> listD = mk.d(assets2);
        for (String str : listHc) {
            if (!hashSet.contains(str)) {
                assets = dVar.d(assets, str, true);
            }
        }
        for (String str2 : listD) {
            if (!isOtherPlugin(str2, strD, strHc) && !hashSet.contains(str2) && !listHc.contains(str2)) {
                assets = dVar.d(assets, str2, false);
            }
        }
        ZeusLogger.i(ZeusLogger.TAG_RESOURCES, "pluginAssets = " + mk.hc(assets));
        return assets;
    }
    ...
    private Resources.NotFoundException handleException(Resources.NotFoundException notFoundException) {
        return new Resources.NotFoundException(("Resources#Assets: " + mk.hc(getAssets())) + "," + notFoundException.getMessage());
    }
}
