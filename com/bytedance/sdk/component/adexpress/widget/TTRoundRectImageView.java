...
public class TTRoundRectImageView extends ImageView {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (drawable != null) {
            Bitmap bitmapD = d(drawable);
            if (bitmapD != null) {
                Shader.TileMode tileMode = Shader.TileMode.REPEAT;
                BitmapShader bitmapShader = new BitmapShader(bitmapD, tileMode, tileMode);
                float fMax = (bitmapD.getWidth() == getWidth() && bitmapD.getHeight() == getHeight()) ? 1.0f : Math.max((getWidth() * 1.0f) / bitmapD.getWidth(), (getHeight() * 1.0f) / bitmapD.getHeight());
                this.c.setScale(fMax, fMax);
                bitmapShader.setLocalMatrix(this.c);
                this.d.setShader(bitmapShader);
                canvas.drawRoundRect(new RectF(0.0f, 0.0f, getWidth(), getHeight()), this.hc, this.b, this.d);
                return;
            }
            super.onDraw(canvas);
            return;
