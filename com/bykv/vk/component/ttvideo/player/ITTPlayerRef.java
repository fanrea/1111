...
public abstract class ITTPlayerRef {
    ...
    public static synchronized ITTPlayerRef create(Context context, TTPlayerClient tTPlayerClient, HashMap<Integer, Integer> map) {
        ...
        if (HardWareInfo.getCpuFamily() == 1 && (tTPlayerRefCreate = TTPlayerRef.create(tTPlayerClient, context)) != null && TTPlayerConfiger.isPrintInfo()) {
            mq.d("ttplayer", "---------->ttplayer on<------------");
        }
        if (tTPlayerRefCreate == null) {
            mq.d("ttplayer", TTPlayerConfiger.getValue(16, "not find start service info."));
