package com.qq.e.comm.plugin;

import android.widget.ImageView;
import com.qq.e.comm.plugin.w40;
import org.json.JSONObject;

/* compiled from: A */
/* loaded from: D:\123Browser\下载\dump_dex_com.awfgwfd.joiyuevgyftrsa\class18.dex */
public class ig extends rq {

    /* compiled from: A */
    public static class b implements w40.e {
        @Override // com.qq.e.comm.plugin.w40.e
        /* renamed from: b, reason: merged with bridge method [inline-methods] */
        public ig a() {
            return new ig();
        }
    }

    private String R() {
        return t10.d() ? "" : "";
    }

    private String S() {
        return t10.d() ? "" : "";
    }

    private ig() {
    }

    /* JADX INFO: Access modifiers changed from: protected */
    @Override // com.qq.e.comm.plugin.rq, com.qq.e.comm.plugin.w40
    /* renamed from: c */
    public sq b(e8 e8Var) {
        sq sqVarB = super.b(e8Var);
        sqVarB.setScaleType(ImageView.ScaleType.FIT_CENTER);
        return sqVarB;
    }

    @Override // com.qq.e.comm.plugin.w40, com.qq.e.comm.plugin.fm
    public void a(String str, JSONObject jSONObject) {
        super.a(str, jSONObject);
        str.hashCode();
        if ((str.equals("closeVideo") || str.equals("closeVideoAuto")) && A() != null) {
            A().setVisibility(8);
        }
    }

    @Override // com.qq.e.comm.plugin.p4, com.qq.e.comm.plugin.w40
    protected boolean a(String str, cm cmVar) {
        str.hashCode();
        if (!str.equals("volume")) {
            return super.a(str, cmVar);
        }
        int iF = cmVar.f(new JSONObject[0]);
        sq sqVarA = A();
        if (iF <= 0) {
            sqVarA.setImageBitmap(gw.a(R()));
            return true;
        }
        sqVarA.setImageBitmap(gw.a(S()));
        return true;
    }
}
