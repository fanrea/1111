...
final class ViewingConditions {
    ...
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        float f16 = 1.0f - f15;
        float fCbrt = (f15 * f) + (0.1f * f16 * f16 * ((float) Math.cbrt(f * 5.0d)));
        ...
        float f19 = fArr7[1];
        return new ViewingConditions(fYFromLstar, ((f18 * 2.0f) + ((f19 * 400.0f) / (f19 + 27.13f)) + (((400.0f * fPow2) / (fPow2 + 27.13f)) * 0.05f)) * fPow, fPow, fPow, f13, f12, fArr6, fCbrt, (float) Math.pow(fCbrt, 0.25d), fSqrt);
    }
