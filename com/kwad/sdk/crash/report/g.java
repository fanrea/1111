...
public final class g extends d {
    public static final Pattern bOD = Pattern.compile("(.*)\\s\\(tid=(\\d+), index=(\\d+)*");
    public static final Pattern bOE = Pattern.compile("\\sd+\\spc");
    ...
    */
    private void a(com.kwad.sdk.crash.model.message.NativeExceptionMessage r4, java.io.File r5) throws java.io.IOException {
        ...
        L11:
            ...
            java.util.regex.Pattern r1 = r3.gY(r1)     // Catch: java.io.IOException -> L7c
    ...
    protected final ExceptionMessage a(File file, File file2, File file3, File file4, File file5) {
        ...
        try {
            ...
            if (file8.exists()) {
                nativeExceptionMessageE.mDumpsys = w.ag(file8);
            } else {
    ...
    */
    ...
    private void a(String str, Backtrace backtrace) {
        ...
        Matcher matcher = gY(".*#\\d+\\s+pc\\s+(\\w+)\\s+.*?([^/]+\\.so)(?:\\s+\\(offset\\s+\\w+\\))?(?:\\s+\\((.*?)\\+\\d+\\))?(\\s+\\(BuildId:\\s+(\\b\\w+)\\))?").matcher(str);
        Matcher matcher2 = gY(".*#\\d+\\s+pc\\s+(\\w+)\\s+.*?([^/]+)\\s+\\((.*?)\\+\\d+\\)\\s+\\(BuildId:\\s+(\\b\\w+)\\)").matcher(str);
        Matcher matcher3 = gY(".*#\\d+\\s+pc\\s+(\\w+)\\s+.*-cache.*\\(offset\\s+\\w+\\)\\s+\\((.*?)\\+\\d+\\)").matcher(str);
        if (matcher.lookingAt() && matcher.groupCount() >= 5) {
            ...
            if (!TextUtils.isEmpty(strGroup2)) {
                if (gY(".*(Mterp|ExecuteMterp|(art::|_ZN3art11)(interpreter|.*Invoke)|art_?interpreter|art_?quick|ZN3art9ArtMethod6Invoke|art::ArtMethod::Invoke).*").matcher(strGroup2).lookingAt()) {
                    nativeBackTraceElement.setNeedClustering(false);
                    return;
                }
                Matcher matcher4 = gY("[^\\[(<\\]]+").matcher(strGroup2);
                if (matcher4.lookingAt()) {
                    Matcher matcher5 = gY(".*(\\b\\w+)").matcher(matcher4.group());
    ...
    private static void a(NativeExceptionMessage nativeExceptionMessage, String str, StringBuilder sb) {
        ...
        if (str.startsWith("pid: ")) {
            ...
            if (strArrSplit2.length >= 9) {
                nativeExceptionMessage.mSignal = com.kwad.sdk.crash.utils.g.he(strArrSplit2[2]);
                nativeExceptionMessage.mCode = com.kwad.sdk.crash.utils.g.he(strArrSplit2[5]);
    ...
    private NativeExceptionMessage E(File file) {
        ...
        if (strAg != null) {
            try {
                nativeExceptionMessage.parseJson(new JSONObject(strAg));
            } catch (Exception e2) {
