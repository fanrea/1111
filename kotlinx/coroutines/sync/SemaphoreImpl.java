...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cont) {
        Object objM2047constructorimpl;
        boolean z;
            ...
            while (true) {
                if (segment2.getId() < j || segment2.getRemoved()) {
                    ...
                    if (obj == ConcurrentLinkedListKt.CLOSED) {
                        objM2047constructorimpl = SegmentOrClosed.m2047constructorimpl(ConcurrentLinkedListKt.CLOSED);
                        break;
                    }
                    ...
                    objM2047constructorimpl = SegmentOrClosed.m2047constructorimpl(segment2);
                    break;
                }
            }
            if (SegmentOrClosed.m2052isClosedimpl(objM2047constructorimpl)) {
                break;
            }
            Segment segmentM2050getSegmentimpl = SegmentOrClosed.m2050getSegmentimpl(objM2047constructorimpl);
            while (true) {
                Segment segment3 = (Segment) this.tail;
                if (segment3.getId() >= segmentM2050getSegmentimpl.getId()) {
                    break;
                }
                if (!segmentM2050getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                    z = false;
                    break;
                }
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(tail$FU, this, segment3, segmentM2050getSegmentimpl)) {
                    if (segment3.decPointers$kotlinx_coroutines_core()) {
                        segment3.remove();
                    }
                } else if (segmentM2050getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                    segmentM2050getSegmentimpl.remove();
                }
            }
            z = true;
        } while (!z);
        SemaphoreSegment semaphoreSegment = (SemaphoreSegment) SegmentOrClosed.m2050getSegmentimpl(objM2047constructorimpl);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        do {
            ...
            while (true) {
                ...
                if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(head$FU, this, segment3, segmentM2050getSegmentimpl)) {
                    if (segment3.decPointers$kotlinx_coroutines_core()) {
                        segment3.remove();
                    }
                } else if (segmentM2050getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
