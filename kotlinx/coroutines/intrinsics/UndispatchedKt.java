        ...
        if (completedExceptionally != IntrinsicsKt.getCOROUTINE_SUSPENDED() && (objMakeCompletingOnce$kotlinx_coroutines_core = scopeCoroutine.makeCompletingOnce$kotlinx_coroutines_core(completedExceptionally)) != JobSupportKt.COMPLETING_WAITING_CHILDREN) {
            if (objMakeCompletingOnce$kotlinx_coroutines_core instanceof CompletedExceptionally) {
                ...
                if (DebugKt.getRECOVER_STACK_TRACES() && (coroutineStackFrame instanceof CoroutineStackFrame)) {
                    throw StackTraceRecoveryKt.recoverFromStackFrame(th3, coroutineStackFrame);
                }
                throw th3;
            }
            ...
            if (objMakeCompletingOnce$kotlinx_coroutines_core instanceof CompletedExceptionally) {
                ...
                if (completedExceptionally instanceof CompletedExceptionally) {
                    ...
                    if (DebugKt.getRECOVER_STACK_TRACES() && (coroutineStackFrame2 instanceof CoroutineStackFrame)) {
                        throw StackTraceRecoveryKt.recoverFromStackFrame(th4, coroutineStackFrame2);
                    }
                    throw th4;
                }
            } else {
        ...
        if (completedExceptionally != IntrinsicsKt.getCOROUTINE_SUSPENDED() && (objMakeCompletingOnce$kotlinx_coroutines_core = scopeCoroutine.makeCompletingOnce$kotlinx_coroutines_core(completedExceptionally)) != JobSupportKt.COMPLETING_WAITING_CHILDREN) {
            if (objMakeCompletingOnce$kotlinx_coroutines_core instanceof CompletedExceptionally) {
                ...
                if (!((Boolean) function1.invoke(completedExceptionally2.cause)).booleanValue()) {
                    ...
                    CoroutineStackFrame coroutineStackFrame = scopeCoroutine.uCont;
                    if (DebugKt.getRECOVER_STACK_TRACES() && (coroutineStackFrame instanceof CoroutineStackFrame)) {
                        throw StackTraceRecoveryKt.recoverFromStackFrame(th2, coroutineStackFrame);
                    }
                    throw th2;
                }
                Throwable th3 = completedExceptionally2.cause;
                CoroutineStackFrame coroutineStackFrame2 = scopeCoroutine.uCont;
                if (DebugKt.getRECOVER_STACK_TRACES() && (coroutineStackFrame2 instanceof CoroutineStackFrame)) {
                    throw StackTraceRecoveryKt.recoverFromStackFrame(th3, coroutineStackFrame2);
                }
                throw th3;
            }
            return JobSupportKt.unboxState(objMakeCompletingOnce$kotlinx_coroutines_core);
    ...
    public static final <T> void startCoroutineUnintercepted(Function1<? super Continuation<? super T>, ? extends Object> function1, Continuation<? super T> continuation) {
        ...
        try {
            ...
            if (objInvoke != IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
                ...
                continuationProbeCoroutineCreated.resumeWith(Result.constructor-impl(objInvoke));
            }
        } catch (Throwable th) {
            Result.Companion companion2 = Result.Companion;
            continuationProbeCoroutineCreated.resumeWith(Result.constructor-impl(ResultKt.createFailure(th)));
        }
    }
