...
public final class ConcurrentLinkedListKt {
    ...
    /* JADX WARN: Type inference failed for: r0v12, types: [kotlinx.coroutines.internal.Segment] */
    /* JADX WARN: Type inference failed for: r0v6, types: [kotlinx.coroutines.internal.Segment] */
    ...
    private static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.getId() >= j && !s.getRemoved()) {
                return SegmentOrClosed.m2047constructorimpl(s);
            }
            Object obj = s.get_next();
            if (obj == CLOSED) {
                return SegmentOrClosed.m2047constructorimpl(CLOSED);
            }
            ?? r0 = (Segment) ((ConcurrentLinkedListNode) obj);
            if (r0 == 0) {
                r0 = (Segment) function2.invoke(Long.valueOf(s.getId() + 1), s);
                if (s.trySetNext((ConcurrentLinkedListNode) r0)) {
                    if (s.getRemoved()) {
                        s.remove();
                    }
                }
